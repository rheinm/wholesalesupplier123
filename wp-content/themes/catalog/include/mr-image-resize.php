...
function catalog_post_thumb( $width=300, $height='', $html=true) {		
  global $post;
  if( !has_post_thumbnail() ) return false;

  $fullsize = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
  $url = catalog_image_resize( $fullsize[0], $width, $height, true, 'c', false );
...
add_action('delete_attachment', 'catalog_delete_resized_images');

function catalog_image_resize($url, $width=null, $height=null, $crop=true, $align='c', $retina=false) {

  global $wpdb;

  // Get common vars (func_get_args() only get specified values)
  $common = catalog_common_info($url, $width, $height, $crop, $align, $retina);
...
function catalog_common_info($url, $width, $height, $crop, $align, $retina) {
...
function catalog_delete_resized_images($post_id) {
