 ...
 */
function ts_hex_shift( $supplied_hex, $shift_method, $percentage = 50 ) {
...
function ts_hex2rgb( $colour, $delimiter = '-' ) {
	if ( $colour[0] == '#' ) {
 ...
 */
function ts_apply_formatting() {
	// Enable shortcodes in text widgets
	add_filter( 'widget_text', 'do_shortcode' );
	// Enable shortcodes in category descriptions
	add_filter( 'category_description', 'do_shortcode' );
	// Enable custom formatting
	if ( get_option( 'ts_option_custom-formatting' ) === 'on' ) {
		// Apply custom formatter function
		add_filter( 'the_content', 'ts_clean_shortcodes' );
	}
}

add_action( 'init', 'ts_apply_formatting' );
 ...
 */
function ts_clean_shortcodes( $content ) {
	$p = ts_cmpt();
	$array = array (
		'<p>[' => '[',
		']</p>' => ']',
 ...
 */
function ts_do_shortcode( $content, $pre ) {
 ...
 */
function ts_compatibility_mode_prefix() {
	return get_option( 'ts_option_prefix' );
}

/**
 * Shortcut for ts_compatibility_mode_prefix()
 */
function ts_cmpt() {
	return ts_compatibility_mode_prefix();
}
 ...
 * @return bool
 */
function ts_addon_active( $addons ) {
	// Prepare add-ons paths
	$paths = array(
		'maker' => 'catalog-maker/catalog-maker.php',
		'skins' => 'catalog-skins/catalog-skins.php',
		'extra' => 'catalog-extra/catalog-extra.php',
	);
...
function ts_skins_link() {
	if ( ts_addon_active( 'skins' ) ) return sprintf( '<br><strong>%s</strong><br><strong>%s</strong>', __( 'Additional skins successfully installed', 'catalog' ), __( 'Open dropdown to choose one of new styles', 'catalog' ) );
	else return sprintf( '<br><a href="http://gndev.info/sus" target="_blank">%s &rarr;</a>', __( 'Get more styles', 'catalog' ) );
}
 ...
 */
function ts_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	global $wp_version;
 ...
 */
function ts_delete_resized_images( $post_id ) {
...
add_action( 'delete_attachment', 'ts_delete_resized_images' );

class Ts_Tools {
	function __construct() {
		add_action( 'wp_ajax_ts_example_preview', array( __CLASS__, 'example' ) );
	...
	public static function get_slides( $args ) {
		...
		if ( $args['gallery'] !== null || ( $args['source'] === 'none' && get_option( 'ts_option_galleries-432' ) ) ) return self::get_slides_432( $args );
		...
		if ( is_array( $query->posts ) ) foreach ( $query->posts as $post ) {
				...
				elseif ( $args['link'] === 'custom' ) $slide['link'] = get_post_meta( $post->ID, 'ts_slide_link', true );
	...
	public static function get_slides_432( $args ) {
		...
		$galleries = get_option( 'ts_option_galleries-432' );
		// No galleries found
	...
	public static function example() {
		...
		self::access();
		// Check nonce
		if ( !isset( $_REQUEST['nonce'] ) || !wp_verify_nonce( $_REQUEST['nonce'], 'ts_examples_nonce' ) ) return;
		// Check incoming data
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="ts-examples-error">' . __( 'Example code does not found, please check it later', 'catalog' ) . '</p>' );
			// Clean-up the code
			$code = str_replace( array( "\t", '%ts_' ), array( '  ', ts_cmpt() ), $code );
			// Split code
			$chunks = explode( '-----', $code );
			// Show snippets
			...
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="ts-examples-code"><span class="ts-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'catalog' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
	...
	public static function reset_examples() {
		foreach ( (array) Ts_Data::examples() as $example ) foreach ( (array) $example['items'] as $item ) delete_transient( 'ts/examples/render/' . $item['id'] );
	}
	...
	public static function icons() {
		$icons = array();
		if ( is_callable( array( 'Ts_Data', 'icons' ) ) ) foreach ( (array) Ts_Data::icons() as $icon ) {
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['ts_slide_link'] = array(
			'label' => __( 'Slide link', 'catalog' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'ts_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'Catalog Shortcodes', 'catalog' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery shortcodes', 'catalog' ) )
		);
		return $form_fields;
	}

	public static function slide_link_save( $post, $attachment ) {
		if ( isset( $attachment['ts_slide_link'] ) )
			update_post_meta( $post['ID'], 'ts_slide_link', $attachment['ts_slide_link'] );
		return $post;
	}

	public static function error( $prefix = false, $message = false ) {
		if ( !$prefix && !$message ) return '';
		$return = array( '<div class="ts-error" style="padding:10px;border:1px solid #f03;color:#903;background:#fde">' );
...
/**
 * Shortcut for Ts_Tools::decode_shortcode()
 */
function ts_scattr( $value ) {
	return Ts_Tools::do_attr( $value );
}

/**
 * Shortcut for Ts_Tools::get_icon()
 */
function ts_get_icon( $args ) {
	return Ts_Tools::get_icon( $args );
}
